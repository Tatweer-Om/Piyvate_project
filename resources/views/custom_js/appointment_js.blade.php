<script>
    ...
    $.ajax({
        ...
        error: function() {
            show_notification('error', '{{ trans('messages.error_fetching_data', [], session('locale')) }}');
        }
    });
}
...
$(document).ready(function () {
    ...
    $("#ministrySelect").change(function () {
        ...
        fetchDetails("{{ url('/getMinistryDetails') }}", ministryId, "#sessionPrice", "#sessionCategory", "#hiddenMinistryPrice");
    });

    // Offer Selection Change Event
    $("#offerSelect").change(function () {
        let offerId = $(this).val();
        fetchDetails("{{ url('/getOfferDetails') }}", offerId, "#offerPrice", "#session_count", "#hiddenOfferPrice");
    });

    // Normal Session Selection Change Event
    $("#sessionSelect").change(function () {
        let sessionId = $(this).val();
        fetchDetails("{{ url('/getsessionDetails') }}", sessionId, "#session_Price", null, "#hiddenSessionPrice");
    });
