    ...
    public function show_ministry_category()
{
    ...
    if (count($view_ministry_category) > 0) {
        ...
        foreach ($view_ministry_category as $value) {
            $colorBox = '<span style="display:inline-block; width:15px; height:15px; background-color:' . $value->ministry_category_color . '; border:1px solid #ccc; margin-right:5px; vertical-align:middle;"></span>';

            $ministry_category_name = '<a href="javascript:void(0);">' . $colorBox . $value->ministry_category_name . '</a>';
    ...
    public function add_ministry_category(Request $request)
    {
        ...
        $ministry_category->ministry_category_color=$request['ministry_category_color'];
    ...
    public function edit_ministry_category(Request $request)
    {
        ...
        $data = [
            ...
            'ministry_category_color' => $ministry_category_data->ministry_category_color,
    ...
    public function update_ministry_category(Request $request)
    {
        ...
        $previousData = $ministry_category->only(['ministry_category_name', 'ministry_category_color', 'branch_id', 'added_by', 'user_id', 'created_at']);
    ...
    public function delete_ministry_category(Request $request)
    {
        ...
        $ministry_category_id = $request->input('id');
        $ministry_category = Ministrycat::where('id', $ministry_category_id)->first();
        $previousData = $ministry_category->only(['ministry_category_name', 'branch_id', 'ministry_category_color', 'added_by', 'user_id', 'created_at']);
